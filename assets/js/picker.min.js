...
function Datepicker(title, cb, o) {
    ...
    function getDay(m, y, endnow) {
        m=parseInt(m);
        y=parseInt(y);
        var D = {1:2,2:0,3:2,4:1,5:2,6:1,7:2,8:2,9:1,10:2,11:1,12:2};
        var day = [];
        if(y&&y%4===0&&m===2) {
            D[2]=-1;
        }
        for(var i=1;i<D[m]+30;i++) {
            var add = true;
            if(endnow && i>new Date().getDate()) {
                add = false;
            }
            if(add)
                day.push({text:(i<10?'0'+i:i)+'', i:(i+100000)});
        }
        return day;
    }
    ...
    if(o.noday)
        ...
        data = [years,months,getDay(1,1)];

    this.instance = new Picker({
        title: title,
        data: data,
        selectedIndex:[(years.length>30?years.length-1:0),0,0]
    });;
    this.instance.on('picker.select', function () {
        var obj = {};
        var names = [];
    ...
    this.instance.on('picker.change', function (index) {
        ...
        if(index===0) {
            if(o.fromnow)
                ...
                this.refillColumn(1, getMonth(null, y.i===new Date().getFullYear()));
            this.scrollColumn(1, 0);

            if(this.data.length>2) {
                this.refillColumn(2, getDay(1,y.text, (o.endnow?a.i-10000===new Date().getMonth()+1:null)));
                this.scrollColumn(2, 0);
            }
        } else if(index===1 && this.data.length>2) {
            this.refillColumn(2, getDay(a.text,y.text, (o.endnow?a.i-10000===new Date().getMonth()+1:null)));
            this.scrollColumn(2, 0);
        }
    });
    ...
    function getMonth(now, endnow) {
        var months = [];
        if(now) {
            var m = new Date().getMonth() + 1;
            for(var i=m;i<13;i++) {
                months.push({text:(i<10?'0'+i:i)+'', i:(i+10000)});
            }
        } else if (endnow) {
