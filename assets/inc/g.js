...
var Comm=new function(){
	...
	var Mess={
		...
		alert:function(mess,code){
			...
			$('#MessgeBoxButton').html('<span class="mbuttonfull" '+callbackstr(1,code)+'>Confirm</span>');
			MainBg.show(true);
			Mess.show=true;
			$(Mess.box).show();
		},
		confirm:function(mess,code){
			...
			$('#MessgeBoxButton').html('<span class="mbuttonl"'+callbackstr(0,code)
				+'>Cancel</span><span class="mbuttonr"'+callbackstr(1,code)+'>Confirm</span>');
			MainBg.show(true);
			Mess.show=true;
			$(Mess.box).show();
		},
	...
	*/
	...
	//注册推送--state:1注册0注销
	z.registerPush=function(jsonObj, cb){
    jsonObj['code']=setcb(cb);
    if(z.w9())callNative('registerPush',jsonObj);
  ...
  /*云信聊天*/
  z.getYXcontact=function(jsonObj){
    if(z.w9()){
      callNative('getYXcontact', jsonObj);
    }
  }
  /*云信聊天登录*/
  z.getlogin=function(jsonObj, cb){
    jsonObj['code']=setcb(cb);
    if(z.w9()){
      callNative('getlogin', jsonObj);
    }
  }
  /*云信聊天推出*/
  z.getYXout=function(jsonObj, cb){
    jsonObj['code']=setcb(cb);
    if(z.w9()){
      callNative('getYXout', jsonObj);
    }
  }
  /*跳地图*/
  z.goMap=function(str){
    if(z.w9())callNative('goMap',str);
  }
  /*地图定位*/
  z.navigation=function(jsonObj){
    if(z.w9()){
      callNative('navigation', jsonObj);
    }
  }
  /*首次定位*/
  z.position=function(cb){
	...
	z.resizeSection=function(){
		var h=Comm.$1('body >header'),f=Comm.$1('body >footer'),s=Comm.$1('body >section');
		if(s){
			s.style.height=(window.innerHeight-(h?h.offsetHeight:0)-(f?f.offsetHeight:0))+'px';
		}
	}
  z.calcSection=function(){
    var s=Comm.$1('body >section');
    if(s){
      if (!s.offsetHeight || s.offsetHeight < 500)
        Comm.resizeSection();
      if(!s.offsetHeight || s.offsetHeight < 500)
        setTimeout(function(){
          Comm.calcSection();
...
window.onload=function(){
  ...
  //  $(".store-window-1").css({"padding-top": "20px"});
  //  Comm.resizeSection();
  //}
