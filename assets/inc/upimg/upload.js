...
function imgUploader(box,maximg,subbutton, addClassClick) {
    //初始化内部变量
    this.imgList=[];
    //this.isUp = true;
    //this.upFlag = true;
    if(arguments.length===1){
        ...
        this.addClassClick = addClassClick;
    }
    ...
    if (Comm.w9()) {
        document.getElementById(this.subbutton).onclick = function () {
            var obj = {url:config.rootjc+'api/imgupload/app/getUploadToken', remain:me.box?(me.maximg-me.imgList.length):1};
            //if (this.isUp == true) {
            //  this.isUp = false;
            //} else {
            //  return;
        ...
        //其他的点击事件点击
        if (this.addClassClick) {
          $("." + this.addClassClick).click(function() {
            var obj = {url:config.rootjc+'api/imgupload/app/getUploadToken', remain:me.box?(me.maximg-me.imgList.length):1};
            Comm.upimg(obj, function (a) {
              //isUp = true;
              var as = a.split(",");
...
imgUploader.prototype.addImgItem = function (src, del) {
    ...
    ibox.className = "box pre";
    ibox.innerHTML = '<img onerror="this.src=\''+this._errorSrc+'\';this.onerror=null" class="upimg" src="' + Comm.OSS.getImgUrl(src,'s',this._errorSrc) + '">' + (del?'<div class="upimg-del" onclick="delete_img(event, \'' + this.subbutton + '\')"></div>':'');
  console.log($(this.box).children("div:last-child").prev());
  if (Comm.w9()) {
    $($(this.box).children("div:last-child").prev()).before(ibox);
  } else {
    $($(this.box).children("div:last-child").prev().prev()).before(ibox);
  }
  $("#" + this.subbutton).find("span").html(this.imgList.length);
  //this.box.appendChild(ibox);
    if (this.imgList.length >= this.maximg) {
      $("#" + this.subbutton).addClass("hide");
    }

    if (this.addClassClick && this.imgList.length <= 0) {
      $("#" + this.subbutton).addClass("hide");
      $(".moxie-shim").addClass("hide");
    } else if (this.addClassClick && this.imgList.length < this.maximg) {
      $("#" + this.subbutton).removeClass("hide");
...
imgUploader.prototype.fill=function (del) {
    ...
    if(this.box) {
        ...
        if(cimg) cimg.src = Comm.OSS.getImgUrl(cimgUrl,'l');
    }
        ...
        init: {
            ...
            FilesAdded: function (up, files) {
                ...
                $('#UpImageBox .select')[0].onclick=function () {
                    ...
                    setMess(true);
                    setMark(100);
                    initParam(up, files[files.length-1].name, false);
                    return false;
                };
            },
...
function get_signature() {
    if (up_expire < Date.parse(new Date()) / 1000 + 3) {
        ...
        serverUrl = config.rootjc + "api/imgupload/getImgPolicy" + "?_token=" + Comm.db("_token");
...
function setMess(l) {
    $('#UpImageBox .upmess').html(l ? 'loading...' : '');
}
