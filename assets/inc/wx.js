var WXAJAX=new function(){
    ...
    function init (post, url, data, cb) {
        ...
        if(!post) {
            ...
            data._t=Math.random();
            if (Comm.db("userinfo")) {
              data._token=Comm.db("userinfo").token;
            }
        }
...
var WXMethod = {
    ...
    wxLogin:function(openid, unionid, nickname, headimgurl, cb) {
        WXAJAX.POST("api/customer/openLogin", {openID:openid, unionid:(unionid?unionid:''), nickName:nickname, avatar:headimgurl}, function (r2) {
